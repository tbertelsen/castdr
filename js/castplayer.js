(function() {
  ...
  CastPlayer.prototype.onInitSuccess = function() {
    ...
    if (autoLaunch) {
      // Delay autolaunch since session may be set right after this call terminates.
      var player = this;
      setTimeout(function() {
        player.tryAutoLaunch();
      }, 300);
    }
  };

  CastPlayer.prototype.tryAutoLaunch = function() {
    console.log("tryAutoLaunch");
    if (this.deviceState == 1) {
      console.log('Skipping launch');
      return;
    } else {
      launchApp();
    }
  ...
  CastPlayer.prototype.onActiveSession = function() {
    ...
    if (autoLaunch) {
      ...
      if (directData) {
        _startPlayback(directData.mediaUrl, directData.title, directData.imgUrl);
      } else {
        startPlayback();
      }
    }
  ...
  CastPlayer.prototype.loadMedia = function(mediaUrl, contentType, title, imgUrl) {
    ...
    if (imgUrl) {
      var image = new chrome.cast.Image(imgUrl);
      image.width = 940;
      image.height = 529;
